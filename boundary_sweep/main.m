function [PSol, WSol] = main(params, idx)
...
P0(1:floor(2*Nx/3))=1.5;
W0(1:floor(2*Nx/3))=1.5;

end

PSol = zeros(Nx+1, Nt+1);
WSol = zeros(Nx+1, Nt+1);
PSol(:,1) = P0;
WSol(:,1) = W0;
...
if isscalar(delta)
    delta_vec = delta*ones(size(W));
else 
    delta_vec = delta(:);
end
rhsW = - (delta_vec + lambdaR).*W - divFluxW;
