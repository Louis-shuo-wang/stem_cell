...
% the next three line is used to simulate degradation-dependent
% rejuvenation strategy
lambdaR = zeros(params.Nx+1,1);
% lambdaR(3*params.Nx/10+1:end) = params.lambdaR;
params.lambdaR = lambdaR;
...
rhsP = -lambdaP*P + lambdaR.*W - divFluxP;
...
rhsW = rhsW + p2*lambdaP * scaling(P, x, gamma1);
rhsW = rhsW + p2*lambdaP * scaling(P, x, gamma2);

% Asymmetric division (p3)
rhsP = rhsP + p3*lambdaP * scaling(P, x, beta1);
rhsW = rhsW + p3*lambdaP * scaling(P, x, beta2);
